...
export function CleanupLogModal({
  ...
  useEffect(() => {
    ...
    if (open && !dialog.open) {
      ...
      // preventScroll avoids iOS shifting the dialog when focus moves
  ...
  return (
    <dialog
      ref={dialogRef}
      className="fixed inset-0 z-50 m-0 flex h-[100dvh] max-h-[100dvh] w-full max-w-none items-center justify-center overflow-hidden border-0 bg-transparent p-4 text-[var(--ink)] open:backdrop:bg-black/40"
    ...
    >
      <div
        ref={panelRef}
        className="flex max-h-[min(90dvh,40rem)] w-full max-w-sm flex-col overflow-y-auto overscroll-contain rounded-lg border border-[var(--line)] bg-white shadow-lg"
        ...
        >
          ...
          >
            ...
            <p id={volumeHelpId} className="mt-2 text-sm text-[var(--mute)]">
              Estimated using 1 litre ≈ 550&nbsp;g of nurdles. Actual weight
              varies depending on debris and moisture.
            </p>
          </fieldset>
          ...
          <p
            id={communityId}
            className="mt-4 text-sm leading-snug text-[var(--mute)]"
          >
            These figures are community contributed and rely on volunteers
            providing their best estimate. They are intended to show the scale
